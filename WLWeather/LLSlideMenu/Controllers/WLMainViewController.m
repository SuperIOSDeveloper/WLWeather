...
- (void)createMaxTempChart{
    ...
    //扩大y值使曲线看起来有坡度
    [maxTempBezierPath moveToPoint:CGPointMake(maxX, ([firstDaily.maxTempC floatValue]*2+40))];
    //最高温label
    self.maxTempLabel.frame = CGRectMake(20, ([firstDaily.maxTempC floatValue]*2+40)-30, 100, 20);
    ...
    for (maxTempDaily in self.dailyArray) {
        [maxTempBezierPath addLineToPoint:CGPointMake(maxX, ([maxTempDaily.maxTempC floatValue]*2+40))];
...
- (void)createMinTempChart{
    ...
    [minTempBezierPath moveToPoint:CGPointMake(minX, ([firstDaily.mintempC floatValue])+80.0)];
    //创建最低温lebel
    self.minTempLabel.frame = CGRectMake(20, (([firstDaily.mintempC floatValue])+80.0)+20, 100, 20);
    ...
    for (minTempDaily in self.dailyArray) {
        [minTempBezierPath addLineToPoint:CGPointMake(minX, ([minTempDaily.mintempC floatValue])+ 80.0)];
